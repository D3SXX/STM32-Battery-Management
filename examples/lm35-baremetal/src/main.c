/*
    Bare metal version of the LM35 temperature sensor reading code. Should be used as reference ONLY (use LL version instead)
*/
  ...
  while (1)
{
	...
	for(int k=0;k<10;k++)
	{
	    raw_value+=read_adc_A1();
	    delay_Ms(50);
	}
	int adc_A1 = raw_value/10;
	if(adc_A1>1241) //1241/4095 x 3.3V = 1V
		GPIOA->ODR|=0x20; //0010 0000 set bit 5. p186
	else
		GPIOA->ODR&=~0x20; //0000 0000 clear bit 5. p186

    float voltage = adc_A1 * (3.3/4095.0);
    ...
    char buf[100];
	sprintf(buf,"A1_ADC=%d Voltage=%d.%d Temp=%d.%d",adc_A1,big_v,(int)small_v, big_temp,small_temp);
	int len=0;
	while(buf[len]!='\0')
	len++;
	for(int i=0;i<len;i++)

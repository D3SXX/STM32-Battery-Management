...
int main(void)
{
  ...
  /* Infinite loop */
	while (1)
	{
		...
		{
			adc_values_A0[k] = read_adc_A0(); // Read A0 and record its output to an array
			LL_mDelay(50); // 50 ms delay for each reading
			adc_values_A1[k] = read_adc_A1();
			LL_mDelay(50); // 50 ms delay for each reading
		}

		int adc_A0 = find_median(adc_values_A0,read_times);
		int adc_A1 = find_median(adc_values_A1,read_times);
		...
		float temp=voltage_A0*100;
		voltage_A0 = voltage_A0 * 1000;
		int volt_integer=(int)voltage_A0;
		int temp_integer=(int)temp;
		int temp_decimals=(int)((temp - temp_integer)*100);
		sprintf(buf,"A0_ADC=%d A0_Voltage=%dmV A0_Temp=%d.%dC",adc_A0,volt_integer, temp_integer,temp_decimals);
		...
		/*Prepare data for USART2 Output (A1)*/
		float voltage_A1 = adc_A1 * (3.3/4095); // Calculate voltage
		temp=voltage_A1*100;
		voltage_A1 = voltage_A1 * 1000;
		volt_integer=(int)voltage_A1;
		temp_integer=(int)temp;
		temp_decimals=(int)((temp - temp_integer)*100);
		sprintf(buf,"A1_ADC=%d A1_Voltage=%dmV A1_Temp=%d.%dC",adc_A1,volt_integer, temp_integer,temp_decimals);

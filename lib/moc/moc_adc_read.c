...
_Atomic uint16_t moc_adc_value = 0;

/**
 * \brief Set return value for moc_adc_read().
 * \param[in] moc_value moc value to be set.
 * \author siyuan xu, e2101066@edu.vamk.fi, Nov.2023
 * \details Define USE_MOC_ADC_SET 1U to enable manual setting, otherwise it generates an
 * sudo-random value.
 */
...
uint16_t moc_adc_read(const uint8_t adc_channel) { return moc_adc_value; }

uint16_t moc_adc_read_cell_voltage(const int cell_index __attribute__((unused))) {
    uint16_t cell_adc  = moc_adc_read(0xff);
...
uint16_t moc_adc_read_batt_current(void) {
    uint16_t current_adc  = moc_adc_read(0xff);
...
uint16_t moc_adc_read_batt_temp(void) {
    uint16_t temp_adc      = moc_adc_read(0xff);
